...
class User extends React.Component {
    ...
    render() {
        ...
        var listAdmins = admins.map(function (admin) {
            return (
                <tr key={admin._id} onClick={()=>this.selectedAdmin(admin)}>
                    ...
                    <td>
                        <button  className="btn blackButton" onClick={(e)=>{e.stopPropagation();this.onDeleteAdmin(admin._id,{role:1})}}>Remove</button>
                    </td>
                </tr>
            );
        }, this);
        return (
            <div>
                <div>
                    <Newadd data={{title:"Add Super Admin",role:"1"}}/>
                    <div className="row" id="title">
                        <div className="col-sm-10" id="userslist">Super Admins</div>
                        <div className="col-sm-2" >
                            <button type="button" className="btn btn-info btn-sm" data-toggle="modal"
                                    data-target="#myUserAddModal">
                                <span className="glyphicon glyphicon-plus"/> Add Super Admin
                            </button>
                        </div>
                    </div>
